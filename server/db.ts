...
export interface AuthChallenge {
  userId: number;
  purpose: "super_admin_mfa_enroll" | "super_admin_mfa_verify";
}
...
async function applySecurityMigrations(): Promise<void> {
  ...
  const superAdminMfaMigrationId = "2026-07-13-super-admin-authenticator";
  const superAdminMfaApplied = await one<{ id: string }>("SELECT id FROM schema_migrations WHERE id = ? LIMIT 1", [superAdminMfaMigrationId]);
  if (!superAdminMfaApplied) {
