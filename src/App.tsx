...
function AuthDialog({ locale, initialAudience, onOpenBloodBankLogin, onClose, onLoggedIn }: { locale: Locale; initialAudience: "personal" | "blood_bank"; onOpenBloodBankLogin?: () => void; onClose: () => void; onLoggedIn: (user: CurrentUser) => void }) {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", role: "requester", bloodGroup: "", rhFactor: "+", district: "", dateOfBirth: "", outreachConsent: false, healthDataConsent: false, answers: {} as Partial<Record<DonorScreeningQuestionKey, DonorScreeningAnswer>> });
  ...
  if (mfaStage !== "credentials") {
    ...
    return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="auth-dialog mfa-dialog" role="dialog" aria-modal="true" aria-labelledby="mfa-title" onMouseDown={(event) => event.stopPropagation()}><button className="dialog-close" onClick={onClose} aria-label="Close sign in">×</button><Pill className="access-pill">SUPER ADMIN AUTHENTICATOR</Pill><h2 id="mfa-title">{enrolling ? "Protect your Super Admin account." : "Confirm Super Admin sign-in."}</h2>{enrolling ? <><p>Scan this code with an authenticator app. This extra step is used only for the Super Admin account, never for Blood Bank branches.</p><div className="mfa-setup"><img src={mfaQrCode} alt="Super Admin authenticator setup QR code" /><div><span>Manual setup key</span><code>{mfaSecret}</code><small>Time-based · 6 digits · 30 seconds</small></div></div></> : <p>Open your authenticator app and enter the current six-digit code to continue.</p>}<form className="auth-form mfa-form" onSubmit={verifyMfa}><label><span>Authenticator code</span><input inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={mfaCode} onChange={(event) => setMfaCode(event.target.value.replace(/\D/g, ""))} required autoFocus /></label>{error && <Notice tone="warning">{error}</Notice>}<button className="button button-signal" type="submit" disabled={working}>{working ? "Verifying…" : enrolling ? "Enable authenticator" : "Verify and sign in"}</button></form></section></div>;
  }
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <p>{isBloodBankStaff ? "Enter the email and temporary password issued by your Super Admin. You will set your own password before accessing Blood Bank data." : mode === "signin" ? "Sign in to continue to your private coordination workspace." : "Requester and donor accounts can be created here. Blood Bank staff accounts are provisioned by an authorized administrator."}</p>
        ...
        <form className="auth-form account-form" onSubmit={submit}>
          ...
          {!isBloodBankStaff && mode === "register" && <><label><span>Account type</span><select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })}><option value="requester">Requester</option><option value="donor">Voluntary donor</option></select></label><label><span>{t(locale, "district")} <em className="required-marker" aria-hidden="true">*</em></span><select value={form.district} onChange={(event) => setForm({ ...form, district: event.target.value })} required><option value="">{t(locale, "chooseDistrict")}</option>{NEPAL_DISTRICTS.map((district) => <option key={district}>{district}</option>)}</select></label>{form.role === "donor" && <><label><span>Self-reported blood group <em className="required-marker" aria-hidden="true">*</em></span><select value={form.bloodGroup} onChange={(event) => setForm({ ...form, bloodGroup: event.target.value })} required><option value="">Choose</option>{groups.map((group) => <option key={group}>{group}</option>)}</select></label><label><span>Rh factor <em className="required-marker" aria-hidden="true">*</em></span><select value={form.rhFactor} onChange={(event) => setForm({ ...form, rhFactor: event.target.value })} required><option value="+">Positive (+)</option><option value="-">Negative (−)</option></select></label><label><span>Date of birth <em className="required-marker" aria-hidden="true">*</em></span><input type="date" value={form.dateOfBirth} onChange={(event) => setForm({ ...form, dateOfBirth: event.target.value })} max={latestEligibleDonorBirthDate()} required /></label><small className="password-guidance full-field">Donors must be at least {DONOR_MINIMUM_AGE}. Your date of birth is kept private and used to derive your age.</small><div className="registration-screening full-field"><b>Required confidential pre-screening</b><small>Answer every question marked <em className="required-marker" aria-hidden="true">*</em> before creating a donor account. A blood-centre clinician makes the final donation decision.</small><div className="screening-question-list">{DONOR_SCREENING_QUESTIONS.map((question) => <label key={question.key}><span>{question.label} <em className="required-marker" aria-hidden="true">*</em></span><select value={form.answers[question.key] ?? ""} onChange={(event) => setForm({ ...form, answers: { ...form.answers, [question.key]: event.target.value as DonorScreeningAnswer } })} required><option value="">Choose a private answer</option><option value="no">No</option><option value="yes">Yes</option><option value="unsure">Unsure</option><option value="not_applicable">Not applicable</option></select></label>)}</div></div><label className="consent-toggle full-field"><input type="checkbox" checked={form.healthDataConsent} onChange={(event) => setForm({ ...form, healthDataConsent: event.target.checked })} required /><span><b>I consent to Raktakosh storing these required answers for confidential facility pre-screening. <em className="required-marker" aria-hidden="true">*</em></b><small>Only authorized facility reviewers can access submitted answers when you have accepted their outreach invitation.</small></span></label><label className="consent-toggle full-field"><input type="checkbox" checked={form.outreachConsent} onChange={(event) => setForm({ ...form, outreachConsent: event.target.checked })} /><span><b>I choose to receive controlled outreach invitations.</b><small>This is optional. You can change or withdraw this preference later.</small></span></label></>}</>}
...
function DonorDashboard({ locale, onMessage }: { locale: Locale; onMessage: (message: string) => void }) {
  ...
  return <div className="dashboard-grid donor-grid">
    ...
    <Card className="screening-card"><form onSubmit={saveScreening}><div className="card-eyebrow">CONFIDENTIAL PRE-SCREENING</div><h2>Share only what a clinician needs to review.</h2><p className="card-intro">This is not medical clearance. Every question marked <em className="required-marker" aria-hidden="true">*</em> and your consent are required before submission. A “yes” or “unsure” response keeps the result in review; a blood-centre clinician makes the final donation decision.</p><div className="screening-status"><span>Current status</span><Pill>{screening.eligibilityStatus.replaceAll("_", " ")}</Pill>{screening.submittedAt && <small>Submitted {formatDate(screening.submittedAt, locale)}</small>}</div><div className="screening-question-list">{DONOR_SCREENING_QUESTIONS.map((question) => <label key={question.key}><span>{question.label} <em className="required-marker" aria-hidden="true">*</em></span><select value={screening.answers[question.key] ?? ""} onChange={(event) => setScreening({ ...screening, answers: { ...screening.answers, [question.key]: event.target.value as DonorScreeningAnswer } })} required><option value="">Choose a private answer</option><option value="no">No</option><option value="yes">Yes</option><option value="unsure">Unsure</option><option value="not_applicable">Not applicable</option></select></label>)}</div><label className="consent-toggle screening-consent"><input type="checkbox" checked={healthDataConsent} onChange={(event) => setHealthDataConsent(event.target.checked)} required /><span><b>I consent to Raktakosh storing these answers for confidential facility pre-screening. <em className="required-marker" aria-hidden="true">*</em></b><small>Only authorized facility reviewers can access submitted answers when you have accepted their outreach invitation.</small></span></label><div className="form-action"><button className="button button-signal" type="submit" disabled={screeningSaving}>{screeningSaving ? "Saving confidential screening…" : "Submit confidential pre-screening"}</button></div></form></Card>
  </div>;
}
...
function AdminDashboard({ locale, onMessage }: { locale: Locale; onMessage: (message: string) => void }) {
  ...
  return <div className="admin-grid">
    ...
    <Card className="admin-staff"><div className="card-eyebrow">STAFF ACCESS</div><h2>Manage tenant staff accounts.</h2><div className="staff-table"><div className="staff-row staff-head"><span>Staff member</span><span>Password setup</span><span>State</span><span>Action</span></div>{overview.staff.map((member) => <div className="staff-row" key={member.id}><div><b>{member.name}</b><small>{roleLabels[member.role]} · {member.facilityName || "Platform"}</small></div><div><Pill className={member.passwordChangeRequired ? "verification suspended" : "verification verified"}>{member.passwordChangeRequired ? "Password change required" : "Password ready"}</Pill>{member.role === "platform_admin" && <small>Authenticator: {member.mfaEnabled ? "enabled" : "required"}</small>}</div><Pill className={`verification ${member.accountStatus === "active" ? "verified" : "suspended"}`}>{member.accountStatus}</Pill><button className="text-button danger-text" disabled={member.accountStatus === "suspended"} onClick={() => void changeStaffStatus(member.id, "suspended")}>Suspend</button></div>)}</div></Card>
