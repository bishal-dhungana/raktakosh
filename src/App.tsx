  ...
  async function load() {
    ...
    try {
      const [requestData, facilityData, platformConfig] = await Promise.all([
        ...
        api<{ documentUploadsEnabled: boolean; documentUploadMessage: string | null }>("/api/public/config")
      ]);
      setRequests(requestData.requests); setFacilities(facilityData.facilities); setDocumentUploadsEnabled(platformConfig.documentUploadsEnabled); setDocumentUploadMessage(platformConfig.documentUploadMessage);
...
function FacilityWorkspace({ user, locale, onMessage }: { user: CurrentUser; locale: Locale; onMessage: (message: string) => void }) {
  ...
  return <div className="facility-workspace">
    ...
    {activePanel === "documents" && operations.privateCaseworkAvailable && <Card className="queue-card"><div className="card-eyebrow">PRIVATE REQUEST DOCUMENTS</div><h2>Review scanned documents before coordination begins.</h2><Notice>Every opening creates an audit event. Files are private, malware-scanned, and opened through a short-lived secure link.</Notice>{operations.documents.length ? <div className="request-stack">{operations.documents.map((document) => { const change = documentReviewChanges[document.id] ?? { reviewStatus: "accepted" as const, reviewNote: "" }; const pending = document.reviewStatus === "pending"; return <article className="request-card" key={document.id}><div className="request-card-top"><div><span className="reference">{document.requestReference}</span><h3>{document.originalName}</h3></div><Pill className="doc-pill">{document.reviewStatus}</Pill></div><div className="request-meta"><span>{document.patientInitials} · {document.urgency}</span><span>{document.mimeType.replace("application/", "")} · {formatBytes(document.byteSize)}</span></div><p className="request-message">Security scan: <b>{document.scanStatus}</b> · Uploaded {formatDate(document.createdAt, locale)} · Retained until {formatDate(document.retentionUntil, locale, false)}</p><div className="review-controls"><button className="button button-ink" onClick={() => void downloadDocument(document.id)}>Open secure document</button>{canReview && pending && <><label><span>Document decision</span><select value={change.reviewStatus} onChange={(event) => setDocumentReviewChanges((current) => ({ ...current, [document.id]: { reviewStatus: event.target.value as "accepted" | "rejected", reviewNote: current[document.id]?.reviewNote ?? "" } }))}><option value="accepted">Accept document</option><option value="rejected">Request replacement</option></select></label><label><span>Review note {change.reviewStatus === "rejected" ? "(required)" : "(optional)"}</span><input value={change.reviewNote} maxLength={500} onChange={(event) => setDocumentReviewChanges((current) => ({ ...current, [document.id]: { reviewStatus: current[document.id]?.reviewStatus ?? "accepted", reviewNote: event.target.value } }))} /></label><button className="button button-signal" onClick={() => void reviewDocument(document.id)}>Save document review</button></>}</div></article>; })}</div> : <EmptyState title="No request documents" body="Documents for cases assigned to this facility will appear here after security scanning." />}</Card>}
    {activePanel === "donors" && operations.privateCaseworkAvailable && <Card className="donor-response-card"><div className="card-eyebrow">CONSENTED DONOR RESPONSES</div><h2>Contact only donors who opted in.</h2><Notice>These details appear only after a donor accepted this facility’s outreach invitation. Do not use them for unrelated contact or export them.</Notice>{operations.donorResponses.length ? <div className="donor-response-list">{operations.donorResponses.map((response) => { const screening = donorScreenings[response.donorUserId]; const review = reviewChanges[response.donorUserId]; return <article key={response.recipientId}><div><Pill className="verification verified">Contact accepted</Pill><h3>{response.donorName}</h3><p>{response.bloodGroup}<sup>{response.rhFactor}</sup> · {response.component} · {response.district}</p><small>Age {response.age ?? "not recorded"} · pre-screening {response.eligibilityStatus.replaceAll("_", " ")}</small><small>For {response.requestReference} · responded {formatDate(response.respondedAt, locale)}</small>{screening && <div className="screening-review"><b>Confidential pre-screening</b>{screening.submittedAt ? <><div className="screening-answer-list">{DONOR_SCREENING_QUESTIONS.map((question) => <p key={question.key}><span>{question.label}</span><strong>{screening.answers[question.key]?.replaceAll("_", " ") ?? "Not answered"}</strong></p>)}</div><div className="screening-review-controls"><label><span>Facility review status</span><select value={review?.eligibilityStatus ?? "provisionally_eligible"} onChange={(event) => setReviewChanges((current) => ({ ...current, [response.donorUserId]: { eligibilityStatus: event.target.value as "needs_review" | "provisionally_eligible" | "not_eligible_now", reviewReason: current[response.donorUserId]?.reviewReason ?? "" } }))}><option value="needs_review">Needs review</option><option value="provisionally_eligible">Provisionally eligible</option><option value="not_eligible_now">Not eligible now</option></select></label><label><span>Review note {review?.eligibilityStatus === "not_eligible_now" ? "(required)" : "(optional)"}</span><input value={review?.reviewReason ?? ""} onChange={(event) => setReviewChanges((current) => ({ ...current, [response.donorUserId]: { eligibilityStatus: current[response.donorUserId]?.eligibilityStatus ?? "provisionally_eligible", reviewReason: event.target.value } }))} maxLength={500} /></label><button className="button button-outline" onClick={() => void saveDonorReview(response.donorUserId)}>Save review</button></div></> : <p className="screening-empty">The donor has not submitted the current confidential pre-screening.</p>}</div>}</div><div className="donor-contact-actions"><a className="button button-ink" href={`tel:${response.phone}`}>Call donor</a><span>{response.phone}</span><small>Preferred: {response.contactWindow}</small><button className="text-button" onClick={() => void viewDonorScreening(response.donorUserId)} disabled={screeningLoading === response.donorUserId}>{screeningLoading === response.donorUserId ? "Loading…" : screening ? "Hide screening" : "Review screening"}</button></div></article>; })}</div> : <EmptyState title="No accepted donor responses" body="Donor details appear here only after a donor accepts a controlled outreach invitation." />}</Card>}
